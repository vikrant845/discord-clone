...
export default async function handler(req: NextApiRequest, res: NextApiResponseServerIo) {
    ...
    try {
        ...
        if (!message || message.deleted) return res.status(404).json('Message Not Found');

        const isOwner = message.memberId === member.id;
        const isAdmin = member.role === MemberRole.ADMIN;
        const isModerator = member.role === MemberRole.MODERATOR;

        const canModifyMessage = isOwner || isAdmin || isModerator;

        if (!canModifyMessage) return res.status(401).json({ error: 'Sorry You Cannot Delete This Message' });
        ...
        if (req.method === 'PATCH') {
            if (!isOwner) return res.status(401).json({ error: 'Sorry You Cannot Edit This Message' });
            message = await db.message.update({
                where: {
                    id: messageId as string,
                },
